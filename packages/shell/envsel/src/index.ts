/**
 * envsel — session environment selector. Per-language slots (`python`, `r`,
 * `cli`) each hold one first-priority environment drawn from conda, standalone
 * R installs, WSL distributions, or user-pinned custom paths. Selections are durable session events
 * (`envsel/selection`, log-only, last-write-wins) surfaced to the model
 * through the runtime-context snapshot, injected into every shell call as
 * `DSH_ENV_*` facts, and changed from the browser via the `/env` command, the
 * `session_env` model tool, or the `envsel` Typert Remote that backs the
 * header dropdown.
 *
 * @module @deepseek-ai/dsh-envsel
 */
...
/** Hard dependencies: every service is shipped in the base layer. */
export const inject = ['subprocess', 'fs', 'systemPrompt', 'shellEnv', 'commands', 'tools', 'timer', 'sessions']
...
/** Schemastery configuration for the envsel consumer. */
export const Config: z<Config> = z.object({
  listTtlMs: z.number().default(LIST_TTL_MS_DEFAULT),
  condaCommand: z.string().default(CONDA_COMMAND_DEFAULT),
  standaloneRRoots: z.array(z.string()).default([]),
  wslEnabled: z.boolean().default(true),
  registerTool: z.boolean().default(true),
  probeTimeoutMs: z.number().default(PROBE_TIMEOUT_MS_DEFAULT),
})
...
/** Validate deployment values at the configuration boundary (schemastery already applied defaults). */
...
export function apply(ctx: Context, config: Config): void {
  ...
  // ── per-session selection: live map, warmed from durable events ─────────────
  ...
  function selectionOfSession(sessionId: SessionId): EnvSelection {
    ...
    const folded = live === undefined ? {} : foldSelection(live.events)
    selections.set(key, folded)
    return folded
  }
  ...
  function setSelectionForSession(sessionId: SessionId, next: EnvSelection): void {
    const live = liveSession(sessionId)
    if (live === undefined) {
      throw new Error(`envsel: session ${String(sessionId)} is not live`)
    }
    live.append('envsel/selection', { slots: next })
    selections.set(sessionId, next)
  }

  function setSelection(agent: Agent, next: EnvSelection): void {
    setSelectionForSession(agent.session.header.id, next)
  }
  ...
  const gatewayHandlers: EnvselGatewayHandlers = {
    ...
    set: async (sessionId: SessionId, slot: EnvSlot, address: string): Promise<EnvselSetResult> => {
      ...
      setSelectionForSession(sessionId, next)
    ...
    try {
      switch (action.kind) {
        ...
        case 'assign': {
          ...
          setSelection(invocation.agent, next)
  ...
  ctx.effect(() => ctx.systemPrompt.context({
    name: 'envsel:selection',
    order: 116,
    text: (context) => {
      const agent = context.agent
      if (agent === undefined) return ''
      return selectionContext(selectionOf(agent))
    },
  }), 'envsel: runtime context')
