/**
 * Shared envsel vocabulary: environment entry shapes, language slots, and the
 * durable per-session selection event. Client-safe — nothing here reaches a
 * Host-only symbol.
 *
 * @module @deepseek-ai/dsh-envsel/types
 */
...
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
    /**
     * The session's envsel slot snapshot after a selection change — log-only
     * and durable (never in the model transcript; the model learns the
     * selection from the runtime-context snapshot). The LAST such event is
     * the session's selection ({@link foldSelection}); consumers read the
     * folded value so restart-safe replay and the live in-memory map agree.
     * `slots` holds one resolved entry per set slot; an empty object means
     * every slot was cleared.
     */
    'envsel/selection': {
      slots: EnvSelection
    }
...
/** Fold the last `envsel/selection` snapshot out of a session event window. */
export function foldSelection(events: readonly { type: string; data: unknown }[]): EnvSelection {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index]
    if (event?.type !== 'envsel/selection') continue
    const data = event.data as { slots?: EnvSelection } | null
    return data !== null && typeof data === 'object' && data.slots !== undefined ? data.slots : {}
  }
  return {}
}

/** True when the selection contains no slot assignments. */
export function isEmptySelection(selection: EnvSelection): boolean {
  return ENV_SLOTS.every(slot => selection[slot] === undefined)
}

/** Stable reference string of an entry, used for display and matching. */
export function entryAddress(entry: EnvEntry): string {
...
export function refMatchesEntry(ref: EnvRef, entry: EnvEntry): boolean {
  if (ref.kind !== entry.kind || ref.name !== entry.name) return false
  if (ref.kind !== 'wsl') return true
  return ref.distro === entry.distro
}

// ── browser Remote vocabulary ────────────────────────────────────────────────
...
export type EnvselSetFailure =
  | EnvselSessionNotFound
  | {
...
export type EnvselUnpinResult =
  | EnvselSuccess<EnvselCatalogValue>
  | EnvselRejected<{ readonly code: 'entry-not-found'; readonly address: string }>
