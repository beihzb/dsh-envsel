...
import { hostCopy, resolveHostLocale, slotLabel } from './copy.ts'
import {
  ENV_SLOTS,
  isEmptySelection,
} from './types.ts'
...
export function parseEnvLine(rawInput: string, locale: 'en' | 'zh' = 'en'): EnvLineAction {
  ...
  if (first === 'add') {
    ...
    if (path.length === 0) return { kind: 'error', text: copy.usageAdd }
    return { kind: 'add', path }
  }
  if (first === 'unpin') {
    const address = tokens.slice(1).join(' ').trim()
    if (address.length === 0) return { kind: 'error', text: copy.usageUnpin }
...
export function envHelpText(locale: 'en' | 'zh' = 'en'): string {
