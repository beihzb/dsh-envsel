...
import { discoverAll, isWindowsHost, probeCustomPath } from "./discover.js";
import { appendPinnedPath, PINNED_FILE_NAME, removePinnedPath, resolvePinnedEntries } from "./pin-cache.js";
import { EnvselRemoteService } from "./remote.js";
import { readAllSelections, writeSessionSelection } from "./state.js";
import { hostCopy, resolveHostLocale, slotLabel } from "./copy.js";
import { ENV_SLOTS, isEmptySelection, } from "./types.js";
export { defaultStandaloneRRoots, isWindowsHost, joinPath, prefixFromRscript, probeCustomPath, } from "./discover.js";
export { parsePinnedDocument, PINNED_FILE_NAME, serializePinnedDocument } from "./pin-cache.js";
export { parseStateDocument, serializeStateDocument, STATE_FILE_NAME } from "./state.js";
export { EnvselRemoteService } from "./remote.js";
...
export function parseEnvLine(rawInput, locale = 'en') {
    ...
    if (first === 'add') {
        ...
        if (path.length === 0)
            return { kind: 'error', text: copy.usageAdd };
        return { kind: 'add', path };
    }
    if (first === 'unpin') {
        const address = tokens.slice(1).join(' ').trim();
        if (address.length === 0)
            return { kind: 'error', text: copy.usageUnpin };
    ...
    for (const token of tokens) {
        ...
        if (match === null) {
            return { kind: 'error', text: copy.unknownArg(token) };
        }
        const slot = match[1];
        const value = match[2] ?? '';
        if (!ENV_SLOTS.includes(slot)) {
            return { kind: 'error', text: copy.unknownSlot(slot) };
        }
...
export function slotCompatible(slot, entry) {
    if (slot === 'python')
...
export function selectionContext(selection) {
    ...
    if (python !== undefined) {
        ...
        if (python.kind === 'wsl')
            lines.push('- WSL: pass Windows paths as /mnt/c/<drive>/<path>.');
...
export function envHelpText(locale = 'en') {
    return hostCopy(locale).help;
}
 ...
 */
export function cliPathGuidance(entry, platform = process.platform) {
    if (entry.kind === 'wsl') {
...
export async function apply(ctx, config) {
    ...
    async function pinPath(path) {
        ...
        if (!probed.ok)
            return rejected({ code: probed.code, path: path.trim() });
        await appendPinnedPath(ctx, path);
        const result = await getCatalog(true);
        return success({ entries: result.entries, warnings: result.warnings });
    }
    async function unpinPath(address) {
        ...
        if (!removed)
            return rejected({ code: 'entry-not-found', address });
        const result = await getCatalog(true);
        return success({ entries: result.entries, warnings: result.warnings });
    }
        ...
        try {
            switch (action.kind) {
                ...
                case 'add': {
                    ...
                    if (!pinned.ok) {
                        const reason = pinned.error.code === 'invalid-path'
                            ? copy.pinReasonInvalid
                            : pinned.error.code === 'not-found'
                                ? copy.pinReasonNotFound
                                : copy.pinReasonNoInterpreter;
                        return { kind: 'error', text: copy.addFailed(action.path, reason) };
                    }
                    const added = pinned.value.entries.filter(entry => entry.kind === 'custom');
                    return {
                        kind: 'success',
                        text: [
                            copy.pinned(pinnedCacheHint()),
                ...
                case 'unpin': {
                    ...
                    if (!removed.ok) {
                        return { kind: 'error', text: copy.unpinMissing(action.address) };
                    }
                    return { kind: 'success', text: copy.unpinned(action.address) };
                }
                case 'list': {
                    const result = await getCatalog(false);
                    ...
                    const entries = filter.length === 0
                        ? result.entries
                        : result.entries.filter(entry => entry.name.toLowerCase().includes(filter) || entry.prefix.toLowerCase().includes(filter));
                    const lines = [
                        copy.listHeader(entries.length, action.filter),
                ...
                case 'assign': {
                    ...
                    for (const assignment of action.assignments) {
                        if (assignment.value.length === 0) {
                            ...
                            applied.push(`${slotLabel(assignment.slot, locale)} → ${copy.slotUnset}`);
                            continue;
                        }
                        ...
                        if (entry === null) {
                            ...
                            const hint = candidates.length > 0
                                ? copy.slotAmbiguous(candidates.map(describeEntry).join(' | '))
                                : copy.slotMissing(assignment.value);
                            return { kind: 'error', text: copy.slotSetFailed(slotLabel(assignment.slot, locale), hint) };
                        }
                        if (!slotCompatible(assignment.slot, entry)) {
                            return {
                                kind: 'error',
                                text: copy.slotIncompatible(slotLabel(assignment.slot, locale), describeEntry(entry)),
                            };
                        }
                        next[assignment.slot] = entry;
                        applied.push(`${slotLabel(assignment.slot, locale)} → ${entry.name} (${entry.kind})`);
                    }
                    await setSelection(invocation.agent, next);
                    return { kind: 'success', text: `${copy.updated}\n${applied.map(line => `  ${line}`).join('\n')}` };
                }
            }
        }
        catch (error) {
        ...
        ctx.tools.register(defineTool({
            ...
            async execute(args, exec) {
                ...
                if (action === 'pin') {
                    ...
                    if (!pinned.ok) {
                        throw new Error(`session_env: cannot add path ${JSON.stringify(path)} (${pinned.error.code})`);
                    }
                ...
                if (action === 'unpin') {
                    ...
                    if (!removed.ok) {
                        throw new Error(`session_env: pinned path not found ${JSON.stringify(address)}`);
                    }
                    return { entries: [...removed.value.entries], warnings: [...removed.value.warnings] };
                }
                if (exec.agent === undefined)
                    throw new Error('session_env requires an owning agent session');
                const agent = exec.agent;
                if (action === 'get')
                    return { selection: selectionOf(agent) };
                if (action === 'set') {
                    ...
                    if (name.length === 0) {
                        ...
                        if (entry === null) {
                            throw new Error(`session_env: environment not found ${JSON.stringify(name)} (use /env list or session_env list)`);
                        }
                        if (!slotCompatible(slot, entry)) {
                            throw new Error(`session_env: ${slot} slot cannot use ${describeEntry(entry)} (missing that language's interpreter)`);
                        }
                        next[slot] = entry;
                    }
