...
import { ENV_SLOTS, ENV_SLOT_LABELS, isEmptySelection, } from "./types.js";
...
export function parseEnvLine(rawInput) {
    ...
    if (first === 'add') {
        ...
        if (path.length === 0)
            return { kind: 'error', text: '用法: /env add <解释器或安装目录的绝对路径>' };
        return { kind: 'add', path };
    }
    if (first === 'unpin') {
        const address = tokens.slice(1).join(' ').trim();
        if (address.length === 0)
            return { kind: 'error', text: '用法: /env unpin custom:<名> 或 /env unpin <路径>' };
