 ...
 */
export function parseStateDocument(raw) {
    let parsed;
    try {
 ...
 */
export function serializeStateDocument(selections) {
    const doc = { version: STATE_FILE_VERSION, selections };
 ...
 * @returns selections keyed by session id.
 */
export async function readAllSelections(ctx) {
    try {
 ...
 */
export async function writeAllSelections(ctx, selections) {
    const target = await ctx.fs.resolve(statePath());
 ...
 */
export async function readSessionSelection(ctx, sessionId) {
 ...
 */
export async function writeSessionSelection(ctx, sessionId, selection) {
    const all = await readAllSelections(ctx);
