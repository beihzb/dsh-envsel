...
import { probeCustomPath } from "./discover.js";
import { writeHomeFile } from "./home-io.js";
 ...
 */
export function pinnedCachePath() {
 ...
 */
export function parsePinnedDocument(raw) {
    let parsed;
    try {
 ...
 */
export function serializePinnedDocument(pins) {
 ...
 */
export async function readPinnedPaths(ctx) {
    try {
 ...
 */
export async function writePinnedPaths(ctx, pins) {
    const target = await ctx.fs.resolve(pinnedCachePath());
 ...
 */
export async function resolvePinnedEntries(ctx) {
 ...
 */
export async function appendPinnedPath(ctx, path) {
    const pins = await readPinnedPaths(ctx);
    const key = path.trim().toLowerCase();
    if (pins.some(pin => pin.path.toLowerCase() === key))
        return;
 ...
 */
export async function removePinnedPath(ctx, addressOrPath, entries) {
    const pins = await readPinnedPaths(ctx);
    const needle = addressOrPath.trim();
    if (needle.length === 0)
        return false;
    const next = pins.filter((pin) => {
        if (pin.path === needle || pin.path.toLowerCase() === needle.toLowerCase())
            return false;
        const match = entries.find(entry => entry.kind === 'custom'
...
//# sourceMappingURL=pin-cache.js.map