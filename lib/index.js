...
*/
function resolveHostLocale(ctx) {
	const settings = ctx.get("settings");
	if (settings === void 0) return "en";
	try {
		const section = settings.get("locale");
		if (section !== null && typeof section === "object" && "preference" in section) return section.preference === "zh" ? "zh" : "en";
	} catch {}
	return "en";
}
...
async function discoverConda(ctx, config) {
	...
	try {
		...
		return {
			entries: [],
			error: copy.condaListFailed(config.condaCommand, String(error))
		};
	}
	if (probe.exitCode !== 0) {
		...
		return {
			entries: [],
			error: copy.condaExit(String(probe.exitCode), detail)
		};
	}
	let parsed;
	try {
...
async function wslDistros(ctx, config) {
	...
	try {
		...
		return {
			distros: [],
			error: copy.wslUnavailable(String(error))
		};
	}
	if (probe.exitCode !== 0) {
		const detail = probe.stderr.trim().slice(0, 300);
		return {
			distros: [],
			error: copy.wslListExit(String(probe.exitCode), detail)
		};
	}
...
async function discoverWslDistro(ctx, config, distro) {
	...
	try {
		...
		return {
			entries: [],
			error: copy.wslProbeFailed(distro, String(error))
		};
	}
	if (probe.exitCode !== 0) return {
		entries: [],
		error: copy.wslProbeExit(distro, String(probe.exitCode), probe.stderr.trim().slice(0, 300))
	};
	...
	try {
		...
		return {
			entries: [],
			error: copy.wslParse(distro, String(error))
		};
	}
	if (parsed === null || typeof parsed !== "object" || !Array.isArray(parsed.envs)) return {
		entries: [],
		error: copy.wslShape(distro)
	};
	const prefixes = parsed.envs.filter((item) => typeof item === "string" && item.length > 0);
	const entries = [];
...
*/
async function writeHomeFile(ctx, target, content) {
	await ctx.fs.writeText(target, content, void 0, void 0, {
...
async function writePinnedPaths(ctx, pins) {
	await writeHomeFile(ctx, await ctx.fs.resolve(pinnedCachePath()), serializePinnedDocument(pins));
}
/**
...
async function writeAllSelections(ctx, selections) {
	await writeHomeFile(ctx, await ctx.fs.resolve(statePath()), serializeStateDocument(selections));
}
/**
...
function parseEnvLine(rawInput, locale = "en") {
	...
	for (const token of tokens) {
		...
		if (match === null) return {
			kind: "error",
			text: copy.unknownArg(token)
		};
		const slot = match[1];
		const value = match[2] ?? "";
		if (!ENV_SLOTS.includes(slot)) return {
			kind: "error",
			text: copy.unknownSlot(slot)
		};
		assignments.push({
			slot,
			value
...
function envHelpText(locale = "en") {
...
async function apply(ctx, config) {
		...
		try {
			switch (action.kind) {
				...
				case "add": {
					...
					if (!pinned.ok) {
						const reason = pinned.error.code === "invalid-path" ? copy.pinReasonInvalid : pinned.error.code === "not-found" ? copy.pinReasonNotFound : copy.pinReasonNoInterpreter;
						return {
							kind: "error",
							text: copy.addFailed(action.path, reason)
						};
					}
					const added = pinned.value.entries.filter((entry) => entry.kind === "custom");
					return {
						kind: "success",
						text: [copy.pinned(pinnedCacheHint()), ...added.map((entry) => `  ${describeEntry(entry)}`)].join("\n")
					};
				}
				case "unpin":
					if (!(await unpinPath(action.address)).ok) return {
						kind: "error",
						text: copy.unpinMissing(action.address)
					};
					return {
						kind: "success",
						text: copy.unpinned(action.address)
					};
				case "list": {
					const result = await getCatalog(false);
				...
				case "assign": {
					...
					for (const assignment of action.assignments) {
						...
						if (entry === null) {
							...
							const hint = candidates.length > 0 ? copy.slotAmbiguous(candidates.map(describeEntry).join(" | ")) : copy.slotMissing(assignment.value);
							return {
								kind: "error",
								text: copy.slotSetFailed(slotLabel(assignment.slot, locale), hint)
							};
						}
	...
	function renderSelection(selection) {
		if (isEmptySelection(selection)) return [copy.noneSelected, copy.noneSelectedHint].join("\n");
		const lines = [copy.currentEnv];
		for (const slot of ENV_SLOTS) {
			const entry = selection[slot];
			const label = slotLabel(slot, locale);
			lines.push(entry === void 0 ? `  ${label} -> ${copy.unsetParen}` : `  ${label} -> ${describeEntry(entry)}`);
		}
	...
	if (resolved.registerTool) ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (action === "pin") {
				...
				if (!pinned.ok) throw new Error(`session_env: cannot add path ${JSON.stringify(path)} (${pinned.error.code})`);
				return {
			...
			if (action === "unpin") {
				...
				if (!removed.ok) throw new Error(`session_env: pinned path not found ${JSON.stringify(address)}`);
				return {
			...
			if (action === "set") {
				...
				if (name.length === 0) delete next[slot];
					...
					if (entry === null) throw new Error(`session_env: environment not found ${JSON.stringify(name)} (use /env list or session_env list)`);
					if (!slotCompatible(slot, entry)) throw new Error(`session_env: ${slot} slot cannot use ${describeEntry(entry)} (missing that language's interpreter)`);
					next[slot] = entry;
				}
